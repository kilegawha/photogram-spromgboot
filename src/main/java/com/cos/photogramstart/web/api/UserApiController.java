...
public class UserApiController {
	...
	public ResponseEntity<?> subscribeList(@PathVariable int pageUserId, 
		...
		return new ResponseEntity<>(
							new CMRespDto<>(1, "구독자 정보리스트 가져오기 성공", subscribeDto), HttpStatus.OK);
	}
	...
	public CMRespDto<?> update(@PathVariable int id, 
			...
			BindingResult bindingResult, // 꼭 @Valid가 적혀있는 다음파라미터에 적어야 된다.
			@AuthenticationPrincipal PrincipalDetails principalDetails) {
			User userEntity = userService.userUpdate(id, userUpdateDto.toEntity());
			principalDetails.setUser(userEntity);
			return new CMRespDto<>(1, "회원수정완료", userEntity);	//응답시에 userEntity의 모든 getter함수가 호출이 되고 Json으로 파싱하여 응답한다.
	}
}
